...
pull_panel_data <- function(progress = NULL) {
  ...
  # F2 is a ~8 MB daily xlsx that causes OOM on shinyapps.io free-tier.
  # Only download when running locally (e.g. via update_cache.R); the
  # deployed app gets bond yields from the bundled cache instead.
  on_shinyapps <- nzchar(Sys.getenv("SHINY_PORT"))
  if (on_shinyapps) {
    if (is.function(progress)) progress(detail = "Skipping F2 (too large for server)")
    gc()
  } else {
